...
export class FormDesignerService {
    ...
    insertForm(obj): Observable<any> {
        return new Observable((observer) => {
            ...
            this.socket.emit('insertForm', { obj: obj }, (response) => {
                if (response.status == 'ok') {
                    // this.selectedForm.next(response.forminserted.formFields);
                    this.WholeForm.getValue().unshift(response.forminserted);
                    ...
                    this.AddForm.next(false);
                    // this.selectedForm.next(undefined);
                    observer.next({ status: 'ok', wholeForm: response.forminserted })
                    observer.complete();
                }
                else {
                    this.snackbar.openFromComponent(ToastNotifications, {
                        data: {
                            img: '/assets/img/icons/svg/warning.svg',
                            msg: 'Error in designing form, Please Try again!'
                        },
                        duration: 2000,
                        panelClass: ['user-alert', 'error']
                    });
